...
public class AdventOfCode11_2 {
    public static void main(String[] args) throws Exception
    {
        ...
        long lcm = 1;
        for( int num = 0; num < monkeys.size(); num++)
            {
            lcm *= monkeys.get(num).getDivisibleTest();
            }
        ...
        for( int rounds = 0; rounds < numRounds; rounds++ )
            ...
            for( int i = 0; i < monkeys.size(); i++ )
                ...
                while( monkeys.get(i).getItems().size() >= 1)
                    {
                    long new_worry = monkeys.get(i).applyOperation(monkeys.get(i).getItems().get(0));
                    int monkey_to_throw_to = monkeys.get(i).getMonkeyToThrowTo(new_worry);
                    monkeys.get(monkey_to_throw_to).AddItem(( new_worry % lcm));
                    monkeys.get(i).RemoveItem();
                    }
                }
            }

        long highest_inspections = 0;
        long second_highest_inspections = 0;
        for( int i = 0; i < monkeys.size(); i++ )
            {
        ...
        long monkey_business_score = highest_inspections * second_highest_inspections;
...
class Monkey {
    ...
    public void AddItem(long item_to_add) {
    ...
    public long applyOperation(long old_worry) {
    ...
    public int getMonkeyToThrowTo(long worry) {
        if( (long)worry % (long)divisible_test == 0)
            {
            return true_monkey;
            }
        else
